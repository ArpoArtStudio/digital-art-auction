...
const checkUserMuteStatus = async (walletAddress) => {
    try {
        const { data, error } = await supabase_1.supabase
            .from('user_mutes')
            .select('muted_until')
...
const setUserMuted = async (walletAddress, duration, adminWallet, reason) => {
    try {
...
const initSocketServer = (httpServer) => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('send-message', async (message) => {
                    ...
                    if (now - userLimit.lastMessageTime > RATE_LIMIT_WINDOW) {
                        ...
                        if (userLimit.messageCount > MAX_MESSAGES_PER_WINDOW) {
                            ...
                            if (userLimit.violations >= 3) {
                                await setUserMuted(walletAddress, 10, // 10 minute duration
                                'system', 'Excessive messaging rate');
                                socket.emit('error', {
            ...
            // Emoji count check
            const emoji = (0, emoji_regex_1.default)();
        ...
        // Admin commands
        socket.on('admin-delete-message', async ({ messageId, adminWallet }) => {
        ...
        socket.on('admin-export-history', async ({ adminWallet }) => {
            // Verify admin
            const isAdmin = adminWallet.toLowerCase() === "0xec24DCDFA7Dc5dc95D18a43FB2A64A23d8E350a0".toLowerCase();
            if (!isAdmin) {
                socket.emit('error', { message: 'Unauthorized admin action' });
