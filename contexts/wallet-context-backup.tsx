...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const displayName = useMemo(() => {
    ...
    switch (displayNameOption) {
      ...
      case DisplayNameOption.USERNAME:
        return user?.username || `${walletAddress.substring(0, 5)}...`
      case DisplayNameOption.WALLET:
      default:
        return `${walletAddress.substring(0, 5)}...`
    }
  }, [walletAddress, displayNameOption, ensName, user])
  ...
  const updateUserProfile = async (updates: Partial<Tables<'users'>>) => {
    ...
    try {
      const { data, error } = await DatabaseService.from('users')
        .update(updates)
        .eq('id', user.id)
        .select()
        .single()
  ...
  // Fetch user data from Supabase on wallet connect
  useEffect(() => {
